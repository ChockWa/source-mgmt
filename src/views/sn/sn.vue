...
export default {
  ...
  methods: {
    getDataList() {
      ...
      this.$api.sn.snsMgmt(this.query).then(resp => {
        if (resp && resp.code === 0) {
          this.dataList = resp.data.sns.records;
          this.totalPage = resp.data.sns.total;
        } else {
    ...
    oneUpload() {
      ...
      _this.$api.source.oneUploadSn({prepareFilePath: _this.targetDir}).then(resp => {
        if(resp){
          if(resp.code === 0){
